...
原始数据文件 `5.20完善数据集.xlsx` 已保留，不应直接覆盖。后续分析、人工校验、论文制图和机器学习数据导出应优先基于 `v0.3_sources_dedup` 版本。
...
当前推荐版本。在 v0.2 基础上对文献来源去重，建立规范来源表 `Source_Registry` 和别名表 `Source_Aliases`。当前统计：
...
3. Biomarker 条目目前是“生物候选”，不是“生命专属分子”。必须进行 targeted nonbiotic search。

4. Ryugu 中 uracil / vitamin B3 等条目需要补充更具体的一手来源。当前主要接到 `Naraoka et al. 2023` 来源域，后续应加入专门的 Ryugu nucleobase/vitamin 文献。

5. Methane pyrolysis 高 MA PAH 目前多为 process/model group-level evidence。正式论文中需要把具体分子名与机制物种标签精确对应。

6. `ML_Dataset_View` 只是候选视图，不能直接作为最终训练集。
