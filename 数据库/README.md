...
原始数据文件 `5.20完善数据集.xlsx` 已保留，不应直接覆盖。后续分析、人工校验、论文制图和机器学习数据导出应优先基于 `v0.6_source_locations` 版本。
...
当前推荐版本。在 v0.5 基础上补充 `Evidence_Log` 的来源链接与可追溯位置，并新增来源位置更新日志和一手文献审计队列。该版本关键状态：
...
- 下一版建议先建立 v0.7 扩库 planning sheet，而不是直接把大数据库全量导入。
